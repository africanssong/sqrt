...
int main()
{
    ...
    m = 12;                       // Fqm에서 m
    t =64;                       // Fqm(X)의 차수 설정. g(x)는 t, Q(x)는 t-1차 다항식.
    //============================================================    
    ...
    for(int x=0;x<m*t-1;x++){
        ...
        for(slong i=0; i < t; i++)
        {
            ...
            {   
                fq_nmod_poly_scalar_mul_fq_nmod(te,xpow[i],qp[i],ctx);
                fq_nmod_poly_add(q2t,q2t,te,ctx);     
